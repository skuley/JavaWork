...
public class String01Main {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		System.out.println("concat()"); // 문자열 연결 / 붙히기 (concatenation)
		...
		str1.concat(str2);
		System.out.println(str1.length()); // concat()을 호출한다고 str1이 변경되지 않는다. 그냥 옆에 붙는다.
		str1 = str1.concat(str2); // 이러면 str1이 새로 변경된다.
...
//		System.out.println(str1.charAt(20)); *** java.lang.StringIndexOutOfBoundsException: String index out of range: 20 ***

		System.out.println();
		System.out.println("indexOf(char), indexOf(String)"); // 문자열 안에서 특정 문자(char) 혹은 문자열(String)의 위치(index), 발견 못하면
																// -1 리턴
		// 이 문자가 몇번째지?? 있나 없나 확인해주고 없으면 -1 을 return한다
		System.out.println(str1.indexOf('c')); // 없으면 -1이 나온다
		...
		System.out.println(url.startsWith(prefix)); // boolean 타입으로 리턴해준다
		...
		System.out.println(str10.replace("My", "Our")); // My를 찾아서 Our로 바꿔준다
		System.out.println(str10.replace("Language", "Java")); // Language를 찾아서 Java로 바꿔준다  (대소문자 구별한다)
