...
// 블럭 밖에서 사용하려 하면 에러 (name is not defined)
// console.log(`name = ${name}`) 
...
{ // Block scope 를 가진 변수는 Block 내에서 사용 가능
    ...
    // console.log('value2 = ', value2) // 여기서는 에러가 난다. 선언을 안해줬기 때문에
}
...
// hoitsing 때문에 동작의 오류처럼 보이는 증상 겪게 됨
...

{
    ...
    // 1 nick 과 2 nick은 변수 scope 가 다르다
}
...
{
    console.log(`2 age = ${age}`); // 블럭 밖 scope 의 변수 사용 가능.
    age = 30;
    console.log(`3 age = ${age}`);
    // var age; // var 는 hoisiting 발생 한다.
    // let age; // let 은 hoisting이 발생 하지 않는다.
}
