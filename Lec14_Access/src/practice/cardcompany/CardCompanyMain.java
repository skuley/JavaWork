...
public class CardCompanyMain {
	...
	public static void main(String[] args) {
		...
		Company company2 = Company.getInstance();// 여러번 호출해도 동일 객체
				
		// ↓ '동일객체' 라고 결과가 나와야 한다.
		System.out.println((com == company) ? "동일객체" : "다른객체");
		
		Card myCard = com.createCard();  // 첫번째 카드생성 고유번호 10001 부여 
		Card yourCard = company.createCard();
		Card hisCard = company2.createCard();
//		
		System.out.println(myCard.getCardNumber());  // 10001 출력
		System.out.println(yourCard.getCardNumber()); // 10002출력	
		System.out.println(hisCard.getCardNumber());
		System.out.println(myCard.getCardNumber());  // 10001 출력
