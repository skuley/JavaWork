...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		regex = "My...."; // My로 시작하고 뒤에 임의의 문자4개의 패턴
		
		// Pattern 객체 생성
		pat = Pattern.compile(regex);
		
		input = "-My1234-"; // 위의 regex 문자열과 패턴이 같다.
		...
		// 위의 코드를 다시 실행하면? 다음 패턴 매칭을 시도한다. --> 결과 실패로 나와야한다.
		...
		// matcher를 reset 시켜주면 다시 패턴 찾기 성공!!
		if(matcher.find()) {
			System.out.println("find() 성공");
			System.out.println(matcher.group() + "{" + matcher.start() + "~" + matcher.end() + "}");
		} else {
		...
		if(Pattern.matches("My....", "Myabc1")) { // 우측에 있는 문자열에 왼쪽에 있는 문자열의 패턴이 있는지 확인한다.
			System.out.println("Pattern.matched() 매칭 OK");
		} else {
			System.out.println("Pattern.matched() 매칭 FAIL");
		} // else
		
		// 정규 표현식에서는 기본적으로 대소문자 구별을 한다.
