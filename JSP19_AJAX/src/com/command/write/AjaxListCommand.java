...
public class AjaxListCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		// parameter 받아오기, 없으면 json 동작 디폴트로 동작하기
		...
		case "xml":
			...
			responseXML2(request, response); // Jackson-databind 사용
			break;

		default:
			//responseJSON(request, response); // org.json 사용
			responseJSON2(request, response); // Jackson-databind 사용
			break;
		} // switch(reqType)
	} // execute()
	...
	private void responseJSON(HttpServletRequest request, HttpServletResponse response) {
		...
		String jsonString = jsonOutput.toString(); // JSON 객체가 String으로 변환
	...
	private void responseJSON2(HttpServletRequest request, HttpServletResponse response) {
		...
		// response 할 Java 객체 생성
		AjaxWriteListJson list = new AjaxWriteListJson();
		...
		if(dtoArr == null) {
			...
			list.setList(Arrays.asList(dtoArr)); // 배열이 list로 바뀐다
		}
		...
		try {
			// java 객체를 json 문자열로 바꿔준다!!
			String jsonStr = mapper.writeValueAsString(list);
			response.setContentType("application/json; charset=utf-8");  // MIME 설정
			response.getWriter().write(jsonStr); // response 보내기
	...
	private void responseXML(HttpServletRequest request, HttpServletResponse response) {
		...
		Element rootElement = new Element("WriteList"); // <WriteList> .. </WriteList> 만들기

		// <WriteList> 가 root 가 된다
		...
		try {
			response.setContentType("application/xml; charset=utf-8"); // text/html도 가능
			// 문자열로 만들어서 response 한다
	...
	private void responseXML2(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String xmlStr = mapper.writeValueAsString(list);
