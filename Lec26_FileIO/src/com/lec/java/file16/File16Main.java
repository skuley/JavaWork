...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		String path = System.getProperty("user.dir") + File.separator + TEST_DIRECTORY; // test 절대 경로
		...
		if(!f.exists()) { // 디렉토리가 존재하는지 체크
			// 디렉토리가 존재하지 않는다면 생성!
			if(f.mkdir()) {// make directory 의 약자 --> mkdir() 성공하면 true, 실패하면 false
				System.out.println("폴더 생성 성공");
			} else {
				System.out.println("폴더 생성 실패"); // 디스크상에 오류가 생기거나 권한이 없거나 용량이 부족하거나 등등..
			}
		} else { // 폴더가 존재한다면
		...
		if(f2.exists()) {
			...
			if(f2.renameTo(f3)) {
				...
				System.out.println("파일 이름 변경 실패"); // 이미 "re_dummy.txt"가 존했으면 실패한다.
			}
		} else {
